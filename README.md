...
The portfolio delivery flow is:

```text
Push portfolio source to main
  → GitHub Actions builds an unprivileged Nginx image
  → Tests check the homepage, résumé PDF, and non-root user
  → Publish commit-tagged image to GHCR and resolve its digest
  → Open a PR updating this repository's portfolio image digest
  → Required checks pass and auto-merge updates main
  → Argo CD reconciles the manifests into Kubernetes
```
